...
def generate_launch_description():
    ...
    defaults = {'profile': str(share/'config/huawei_v34_left_sim.json'),
                ...
                'output': '/tmp/bindu-runs', 'vr_enabled': 'false',
    ...
    nodes = []
    for executable, keys in {'execution': (), 'recorder': ('output',), 'teleop': ('teleop_config',)}.items():
        nodes.append(Node(package='bindu_runtime', executable=executable,
                          namespace=LaunchConfiguration('namespace'), output='screen',
                          parameters=[{**common, **{k:LaunchConfiguration(k) for k in keys}}]))
