...
[返回项目首页](README.md) · 首次使用先完成[环境准备与快速开始](README.md#环境要求)。以下命令默认在仓库根目录执行，并按各节要求加载 ROS 与工作区环境。

本文集中维护专项运行命令、配置说明及验证边界。带日期的结果为对应历史批次，不能替代当前版本验收；原始实验数据保存在主开发工作区的 `artifacts/`，不随 Git 分发。后续日常实验进展记入本地进度与过程记录，本文仅在使用方式或验证边界变化时更新。

## 按需查阅

- [Conda 兼容性](#conda-兼容性) · [日志记录级别](#日志记录级别) · [通用测试](#测试)
- [导航协议验证](#导航适配验证入口) · [G1 全身模拟](#临时-g1-全身模拟入口)
- [Isaac 物理仿真](#isaac-sim-图形物理仿真) · [Nav2 房间导航](#真实-nav2-与-isaac-房间导航) · [二维建图与定位](#isaac-内的实际建图与地图定位)
- [三维导航迁移](#历史三维导航迁移入口) · [三维物理复测](#三维算法的-isaac-同场景复测)
- [Pi 客户端](#pi-客户端运行入口) · [VR 遥操作](#vr-遥操作运行入口) · [One Euro 滤波](#vr-位姿-one-euro-滤波)
- [共享执行层](#共享执行层验证) · [开发同步](#开发同步)
...
在按[快速开始](README.md#快速开始)完成依赖安装/构建的 Ubuntu ROS 环境中运行：
...
可选依赖在 Ubuntu 24.04 / Python 3.12 / x86_64 验证；Orin 尚未验证。先按[快速开始](README.md#快速开始)构建并加载 ROS 与工作区，再在当前工作区隔离安装依赖。下面的库路径用于避免 ROS 自带 EigenPy 与 Pinocchio wheel 混用，仅影响当前终端：
