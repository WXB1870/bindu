...
class NavigationSensors:
    def __init__(self, node, world, config, robot, excluded_path):
        ...
        self.static.sendTransform([t,identity])
        ...
        self.fault=node.create_subscription(String,'navigation/sensor_fault',
            lambda m:setattr(self,'pose_enabled',m.data!='pose_loss'),1)
    ...
    def publish(self, feedback):
        ...
        # Ideal simulator odometry, no integration drift or localization claim.
        ...
        t.transform.translation.x=p.position.x; t.transform.translation.y=p.position.y
        ...
        if now-self.last_scan<1/laser['rate']: return
        self.last_scan=now
        ...
        for i in range(laser['beams']):
            ...
            ranges.append(distance if distance>=scan.range_min else float('inf'))
