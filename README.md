...
| VR 遥操作与 IK | 已接 v3.4 手柄输入、相对控制和连续 IK；新增单臂模拟末端/状态可视反馈，本批 ROS 联调待验证 |
| 语音交互 | 已预留模块目录 |
...
专项验证启动自己的模拟进程：覆盖启动入口、合成手柄事件经真实 Vuer WebSocket 服务接入、暂停/重接、取消、断流、追踪无效、连接身份改变、不可达目标和记录器退出；新增显示状态与显示进程退出隔离检查（本批尚未在 ROS 环境执行）。真实头显的浏览器、TLS、跟踪质量与操控感受仍需现场联调。
