...
def run_case(root,output,case):
    ...
    try:
        ...
        goal=wait(node,action.send_goal_async(FetchDrink.Goal(task_id=run,object_id='drink',strategy='planner')))
...
def main():
    ...
    try:
        for case in args.cases:
            try:
                result=run_case(Path(__file__).resolve().parents[1],args.output,case);result['passed']=True
